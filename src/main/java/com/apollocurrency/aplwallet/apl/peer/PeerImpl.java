...
final class PeerImpl implements Peer {
    ...
    void setVersion(Version version) {
        ...
        isOldVersion = false;
        if (Apl.APPLICATION.equals(application)) {
    ...
    public void blacklist(String cause) {
        blacklistingTime = Apl.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/apl"));
            //
    ...
    void connect() {
        lastConnectAttempt = Apl.getEpochTime();
        try {
